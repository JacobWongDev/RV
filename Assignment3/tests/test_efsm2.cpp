...
int main() {
    ...
    std::vector<char> alphabet = {'a', 'b', 'c', 'd', 'f', 'g'};
    // Initialize states
    State q1("q1", START);
    states.push_back(&q1);
    State q2("q2", DEFAULT);
    ...
    ASSERT("'badaaaaf'", complement->verify("badaaaaf"), false);
    ASSERT("'fabc'", complement->verify("fabc"), true);
    ASSERT("'ccdf'", complement->verify("ccdf"), true);
    return 0;
}